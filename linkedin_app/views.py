...
from wyge.agents.prebuilt_agents import ResearchAgent, LinkedInAgent, VideoAudioBlogAgent, YTBlogAgent
import json
    ...
    if request.method == 'POST':
        ...
        try:
            if post_type == 'website':
                ...
                context = research_agent.research_website(topic, url)
                ...
                yt_agent = ResearchAgent(api_key)
                context = yt_agent.extract_transcript_from_yt_video()
                print("Context from YTBlogAgent:", context)  # Debug print
                content, image_path = linkedin_agent.generate_linkedin_post(context)

            elif post_type == 'video/audio':
                file_path = data.get('file_path')
                va_agent = ResearchAgent(api_key)
                context = va_agent.extract_text_from_audio_or_video(file_path)
