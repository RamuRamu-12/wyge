...
# def generate_data_from_text(api_key, text_sample, num_rows=10, chunk_size=50):
    
#     llm = ChatOpenAI(api_key)
    
#     sysp = "You are a data generator that produces only CSV formatted data with no extra text or code fences."
...
#         else:
#             current_sample_str = text_sample

#         rows_to_generate = min(chunk_size, num_rows - rows_generated)

#         # prompt = (f"Generate {rows_to_generate} rows of synthetic data following this pattern:\n\n{current_sample_str}\n"
#         #           "\nMake sure not to repeat any data and output it only in comma-separated values (CSV) format, "
