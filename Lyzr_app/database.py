...
class PostgreSQLDB:
    ...
    #Create Environment
    def create_environment(self, name, model_vendor, api_key, model, temperature=0.5, top_p=0.9):
        try:
            conn = self.connect()
            if conn is not None:
                cursor = conn.cursor()
                query = """
                INSERT INTO environment (name, model_vendor, api_key, model, temperature, top_p)
                VALUES (%s, %s, %s, %s, %s, %s)
                RETURNING id;
                """
                cursor.execute(query, (name, model_vendor, api_key, model, temperature, top_p))
    ...
    def update_environment(self, environment_id, name=None, model_vendor=None, api_key=None, model=None,
        ...
        try:
            ...
            if conn is not None:
                ...
                WHERE id = %s;
                """
                cursor.execute(query, (name, model_vendor, api_key, model, temperature, top_p, environment_id))
                conn.commit()
                cursor.close()
                conn.close()
            ...
            if conn is not None:
                ...
                if environments:
                    for environment in environments:
                        print(
                            f"ID: {environment[0]}, Name: {environment[1]}, Model Vendor: {environment[2]}, Model: {environment[4]}, Temperature: {environment[5]}, Top P: {environment[6]}")
                    return environments
                else:
