    ...
    try:
        ...
        if file and 'text_to_sql' in agent[4]:
            ...
            if isinstance(result, dict):
                ...
                # Return HTML response for text_to_sql
            ...
            if isinstance(result, dict):
                ...
                # Return HTML response for URL-based generation
                return HttpResponse(f"""
                        <html>
                            <body>
                                <div class="p-6 bg-blue-50 rounded-lg text-center shadow-lg">
            ...
            if isinstance(result, dict):
                ...
                # Return HTML response for file-based generation
                return HttpResponse(f"""
                        <html>
                            <body>
                                <div class="p-6 bg-blue-50 rounded-lg text-center shadow-lg">
            ...
            return HttpResponse("""
                    <html>
                        <body>
                            <div class="p-6 bg-red-50 rounded-lg text-center shadow-lg">
                                <h2 class="text-2xl font-semibold text-red-600">⚠️ No Content Generated ⚠️</h2>
                                <p class="text-lg text-gray-700 mt-2">Please provide valid input for processing.</p>
                            </div>
                        </body>
                    </html>
                """)
...
# Generate content from URL (for blog or LinkedIn post)
def generate_blog_from_yt_url(prompt, yt_url, option, api_key):
    try:
        if option == 'blog_post':
            yt_agent = YTBlogAgent(api_key)
            blog,doc_file,image = yt_agent.generate_blog(yt_url)
