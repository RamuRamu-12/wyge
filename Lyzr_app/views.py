...
def run_openai_environment(request):
    try:
        ...
        if user_prompt and 'chat_app_text' in agent[4]:
            result = chat_with_openai(openai_api_key, user_prompt, temperature, audio_response=False)
            ...
            if "error" in result:
                ...
                # if audio_response:
                #     response_data["audio"] = result.get("audio")

        # Audio_transcription chat_app
        if file and 'transcribe_audio' in agent[4]:
            result = transcribe_audio(openai_api_key, file)
...
# Transcribe audio for chat_app
def transcribe_audio(api_key, audio_file):
    if not audio_file:
        return {"error": "No audio file provided"}

    # Convert InMemoryUploadedFile to bytes
    file_bytes = audio_file.read()  # Read file contents into bytes

    # Prepare file as a tuple with (filename, file_bytes)
    file_tuple = (audio_file.name, file_bytes)
    ...
    try:
        ...
        # Check the structure of transcript and access the 'text' field properly
