    ...
    try:
        ...
        if file and 'text_to_sql' in agent[4]:
            ...
            if isinstance(result, dict):
                # Populate response_data based on result's keys
            ...
            if isinstance(result, dict):
                response_data["content"] = result.get("content", "")
                if "image_path" in result and result["image_path"]:
                    response_data["image_base64"] = image_to_base64(result["image_path"])

        elif yt_url and user_prompt:
            if 'blog_post' in agent[4]:
                result = generate_blog_from_yt_url(user_prompt, yt_url, 'blog_post', openai_api_key)
            elif 'linkedin_post' in agent[4]:
                result = generate_blog_from_yt_url(user_prompt, yt_url, 'linkedin_post', openai_api_key)
            if isinstance(result, dict):
                response_data["content"] = result.get("content", "")
                if "image_path" in result and result["image_path"]:
                    response_data["image_base64"] = image_to_base64(result["image_path"])
